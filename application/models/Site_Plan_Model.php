...
class Site_Plan_Model extends CI_Model

{
...

	{
		...
		$query = $this->db->select('r.id as userid, r.fullname, r.mobile, r.email, r.city, r.state, r.isUser, r.cardtype, r.process_step, a.id, a.loantype, a.loanamount, a.income, a.currentemi')
			...
			->join('plan_user_registration r', 'r.id=a.userid')
		...
		switch ($loantype) {
			...
			case '21':
				...
				if ($mobile != '') {
					...
					$message = $this->Site_Info_Model->getsmsmessage('plan-process-sms');
		...
		switch ($loantype) {
			...
			case '21':
				...
				if ($mobile != '' && $eligibilityamt != 0) {
					...
					$message = $this->Site_Info_Model->getsmsmessage('plan-offer-sms');
			...
			case '22':
				...
				if ($mobile != '' && $eligibilityamt != 0) {
					...
					$message = $this->Site_Info_Model->getsmsmessage('plan-bl-offer-sms');
...

	{
		...
		if ($mobile != '') {
			...
			$message = $this->Site_Info_Model->getsmsmessage('plan-payment-fail-sms');
...

	{
		...
		if ($emailid != '') {
			...
			$content = $this->Site_General_Model->plan_welcomeemailtemplate($mobile, $password);
