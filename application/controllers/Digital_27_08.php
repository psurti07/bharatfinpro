...
class Digital extends CI_Controller
{
	...
	public function membershiporder($key = '')
	{
		...
		if ($userdate == NULL) {
			...
			$this->load->view('digital-membership-card', ['meta' => $meta, 'userdetails' => $userdata, 'productdata' => $productdata, 'banklist' => $banklist]);
		}
	}
	...
	public function getpreApproval()
	{
		...
		$data4 = array(
			...
			"template"=> array(
					...
					"languageCode"=> "en",
					"headerValues"=> array(
						"https://interaktprodmediastorage.blob.core.windows.net/mediaprodstoragecontainer/f64c85fc-b4b5-470a-be94-9f25a4b933e5/message_template_media/nwjyez4j3rrG/flexi26aug.jpg?se=2029-08-20T11%3A41%3A25Z&sp=rt&sv=2019-12-12&sr=b&sig=aef5ikdb4yRQDWQERrTweHYObkydGcual3FNuAqdvY8%3D"
					),
					"bodyValues"=> array(
	...
	public function checkoutDigital()
	{
		...
		$failURL = base_url('digital/membershiporder/' . $key);
	...
	public function buycardDigital() {
		...
		if (isset($_REQUEST['paymentid']) && $_REQUEST['paymentid'] != '') {
			...
			if ($isentry == 0) {
				...
				redirect("digital/paymentResponse/" . $userdata->cardtype . '/' . $response2);
			} else {
				return redirect("digital/paymentResponse/" . $userdata->cardtype . "/true");
				die;
			}
		} else {
			$key = stringCrypt($_REQUEST['applyid'], 'encrypt');
