...
class Digital extends CI_Controller
{
	...
	public function checkoutDigital()
	{
		...
		$returnUrl = base_url('digital/buycardDigital?orderid=' . $orderid);
	...
	public function buycardDigital()
	{
		...
		if (isset($_REQUEST['orderid'])) {
			...
			if ($orderdata) {
				...
				if ($txstatus == 'SUCCESS') {
					$isentry = $this->Site_Digital_Model->checkmembershipentry($referenceid);
					...
					if ($isentry == 0) {
						...
						$response3 = $this->Site_Digital_Model->updateapplication($userdata->id, $appdata);
						...
						return redirect("digital/paymentResponse/" . $userdata->cardtype . "/" .$response2);
						die;
					} else {
						return redirect("digital/paymentResponse/11/true");
						die;
					}
				} 
