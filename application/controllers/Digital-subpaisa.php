...
class Digital extends CI_Controller
{
	...
	public function preapproval($key = '')
	{
		...
		if ($userdate == NULL) {
			...
			$this->load->view('digital-pre-approval', ['meta' => $meta, 'userdetails' => $data]);
		}
	}
	...
	public function getpreApproval()
	{
		...
		$offerresponse = $this->Site_Digital_Model->sendOfferMessage($userdata->loantype, $userdata->mobile, $userdata->email);
	...
	public function paymentResponse($loantype = '', $status = '')
	{
		...
		if ($status != '') {
			if ($status == "true" && $this->session->tempdata('applyid') != "") {
				...
				$fullname = strtok($userdata->fullname, " ");
				...
				$fbdata = array(
					...
					'mobile' => hash("sha256", '91' . $userdata->mobile),
					'email' => hash("sha256", $userdata->email),
					'city' => hash("sha256", $city),
					'state' => hash("sha256", $state),
					'orderid' => $orderid,
					'sourceurl' => base_url('digital/paymentResponse/' . $loantype . '/true')
				);
