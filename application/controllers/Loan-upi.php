...
class Loan extends CI_Controller
{
	...
	public function getcardoffer()
	{
		...
		$this->load->model('Site_Digital_Model');
		$steptopaymentry = $this->Site_Digital_Model->steptopayentry($steppaydata);

		$this->load->helper('steptopay');
		$resprofile = generate_bearer();

		$orderdata = array(
			'billing_person_name' => $fullname,
		...
		$resorder = create_order($orderdata, $resprofile['access_token']);

		echo "<a href='".$resorder['upi_url']."'>Click here</a>";
		
		die;

		/*  if($resorder) {
			header("location:" . $resorder['upi_url']);
			die;
		}
		else {
