...
class Digital extends CI_Controller
{
	...
	public function checkoutDigital()
	{
		...
		$post_data->p_info = "Membership Card Purchase";
		$post_data->customer_name = $userdata->fullname;
		$post_data->customer_email = $userdata->email;
		$post_data->customer_mobile = $userdata->mobile;
		...
		if (isset($_GET['client_txn_id'])) {
			...
			if ($response['status'] == true) {
				...
				$restrack4 = interakt_track($data_usr_pass);
					...
					$this->Site_Digital_Model->sendSuccessGreetings($userdata->mobile, $userdata->email, $password);
