...
window.onload = function() {
    document.getElementById("1600").className += " active";
}
</script>

<div class="content-header row">
    <div class="content-header-left col-md-12 col-12 mb-1">
        <h1 class="content-header-title text-uppercase">Webinar Customers</h1>
    </div>
</div>
                ...
                <div class="card">
                    ...
                    <div class="card-header">
                        <div class="heading-elements">
                            <?php echo form_open('webinar', array('id' => 'filterForm', 'class' => 'form-horizontal', 'novalidate' => 'novalidate')); ?>
                                ...
                                <tbody>
                                    ...
                                    if (count($userlist)) {
                                        ...
                                        foreach ($userlist as $row) {
                                            ...
                                            echo "<td class='dont-break-out'>" . htmlentities($row->email) . "</td>";
                                            echo "<td>" . htmlentities($row->city) . "</td>";
                                            echo "<td>" . htmlentities($row->state) . "</td>";
                                            if ($row->is_joincommunity == 0) {
                                                echo "<td>" . anchor("webinar/join_community/{$row->program_id}/{$row->id}", 'Not Joined', 'class="btn btn-icon btn-outline-danger btn-sm"') . "</td>";
                                            } else if ($row->is_joincommunity == 1) {
                                                echo "<td>" . anchor("webinar/join_community/{$row->program_id}/{$row->id}", 'Joined', 'class="btn btn-icon btn-outline-success btn-sm"') . "</td>";
                                            }
                                            //echo "<td class='text-center'>".anchor("webinar/webinar_userdetail/{$row->program_id}/{$row->id}",'<i class="la la-info"></i>','class="btn btn-icon btn-outline-dark btn-sm"')."</td>";
                ...
                <div class="card shadow-sm border-0 mb-4">
                    <div class="card-body p-4">
                        ...
                        <div class="row mt-3 align-items-center">
                            <div class="col-md-2">
                                <label class="fw-bold">Schedule Link:</label>

                            </div>

                            <div class="col-md-8">
                                <input type="text" id="schedule_link" class="form-control" readonly>
                            </div>

                            <div class="col-md-2">
                                <button class="btn btn-success w-100" id="copyBtn" data-bs-toggle="tooltip"
                                    data-bs-placement="top" title="Copy Community Link">
                                    <i class="la la-copy">Copy</i>
                                </button>
                            </div>
                        </div>
                    </div>
...
$(document).on('click', '.viewDetails', function() {
    ...
    $.ajax({
        url: "<?php echo site_url('webinar/webinar_userdetail'); ?>",
        type: "POST",
        dataType: "json",
        data: {
...
$('#copyBtn').click(function() {
    var copyText = document.getElementById("schedule_link");
